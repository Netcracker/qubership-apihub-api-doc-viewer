...
import { NestingIndicatorTitleRowContent } from "./NestingIndicatorTitleRowContent"
import type { NestingIndicatorTitleRowProps } from "./types"

export const NestingIndicatorTitleRow: FC<NestingIndicatorTitleRowProps> = memo<NestingIndicatorTitleRowProps>((props) => {
  const layoutMode = useLayoutMode()

  const {
    diffsSeverities,
    diffsSeverityPlacement = NodeDiffsSeverityPlacemennt.TitleRow,
  } = props
  ...
  switch (layoutMode) {
    case SIDE_BY_SIDE_DIFFS_LAYOUT_MODE:
      return (
        ...
        >
          <SideBySideLayout
            left={<NestingIndicatorTitleRowContent {...props} layoutSide={ORIGIN_LAYOUT_SIDE} />}
            right={<NestingIndicatorTitleRowContent {...props} layoutSide={CHANGED_LAYOUT_SIDE} />}
          />
        </DiffFloatingBadgeWrapper>
      )
    case DOCUMENT_LAYOUT_MODE:
      return (
        <OneSideLayout
          content={<NestingIndicatorTitleRowContent {...props} layoutSide={CHANGED_LAYOUT_SIDE} />}
        />
      )
  }
