...
const printStoryFile = (suite, suiteCases) => {
  ...
  const factoryName = suite.includesOneOfOrAnyOfCombiner
...
const printTestFile = (suite, caseIds) => {
  ...
  const combinerImport = suite.includesOneOfOrAnyOfCombiner
    ? `\nimport { switchCombinerNodesToChangedVariant } from "../../utils/combiner-changed-variant";`
    : "";
  const combinerSwitchLine = suite.includesOneOfOrAnyOfCombiner
  ...
  it("${caseId}", async () => {
    ...
    await waitForJsonSchemaDiffViewer();${combinerSwitchLine}
