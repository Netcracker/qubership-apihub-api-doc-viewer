        ...
        '200':
          ...
          headers:
            X-Header-1:
              schema:
                type: [string, "null"]
`}]]),Gn=Symbol("synthetic-title"),Qn={validate:!0,liftCombiners:!0,syntheticTitleFlag:Gn,unify:!0,allowNotValidSyntheticChanges:!0},Mn=(n,t)=>{const e=un(n,t,{...Qn,beforeSource:n,afterSource:t,mode:dn,metaKey:sn});return _n(e.merged)&&(e.merged.toJSON=()=>pn(e.merged)),e};function _n(n){return n!=null&&typeof n=="object"}const Cn=["Inter"];function Xn({before:n,after:t}){const{diffs:s,merged:o}=Mn(H(n),H(t)),[e,p]=bn.useState(!1),c=Cn.map(a=>new Rn(a).load(null,1e4));return Promise.all(c).then(()=>{p(!0)}),e?(console.log("apihub-diff:",pn(s.map(a=>{const{beforeDeclarationPaths:y=[],afterDeclarationPaths:h=[],...D}={...a};return{...D,...N.isNotEmpty(y)?{beforeDeclarationPaths:`[${y.map(I=>`[${I.join()}]`).join()}]`}:{},...N.isNotEmpty(h)?{afterDeclarationPaths:`[${h.map(I=>`[${I.join()}]`).join()}]`}:{}}}))),Un(n,t),Q.jsx(vn,{layoutMode:hn,diffMetaKey:sn,source:o})):Q.jsx(Q.Fragment,{})}function Un(n,t){const s=J(n,{noLocation:!0}),o=J(t,{noLocation:!0}),e=Pn(s,o),p=An(s,o);console.log("GraphQL Changes:"),N.isNotEmpty(e)?console.log("Breaking Changes:",e.map(c=>`
${c.description}`).join()):console.log("No Breaking Changes"),N.isNotEmpty(p)?console.log("Dangerous Changes:",p.map(c=>`
