import{j as B}from"./_commonjs-dynamic-modules-6308e768.js";import{c as tn,P as rn,D as C,s as sn,S as pn,a as on}from"./ErrorBoundaryFallback-bacde49f.js";import{g as an,r as mn}from"./index-f46741a2.js";import{G as cn}from"./GraphQLOperationDiffViewer-27890bb8.js";import{b as R}from"./graph-api-transformers-169acba1.js";const X=new Map([["graphql/directives/add-argument-default-value-for-schema-directive",{before:`directive @limit (offset: Int) on FIELD_DEFINITION
...
`},after:{schema:`type: number
maximum: 99
`}}],["add-min-items-for-array-property",{before:{schema:`properties:
  option1:
    type: array
    minItems: 0
    items:
...
`},after:{schema:`properties:
  ...
  option2:
    type: string
    minLength: 1
`}}],["add-min-properties-for-object-property",{before:{schema:`properties:
  option1:
    minProperties: 0
    properties:
      prop1:
...
type: string
`}}],["add-sibling-description-for-ref",{before:{schema:`$ref: "#/components/schemas/Pet"
`},after:{schema:`$ref: "#/components/schemas/Pet"
description: schema description override
...
`},after:{schema:`properties:
  ...
  option2:
    type: array
    uniqueItems: false
`}}],["change-referenced-enum-when-sibling-exists-for-ref",{before:{schema:`$ref: "#/components/schemas/Color"
enum:
  - black
  - white
`},after:{schema:`$ref: "#/components/schemas/Color"
enum:
  - black
  - white
`}}],["change-sibling-enum-for-ref",{before:{schema:`$ref: "#/components/schemas/Color"
enum:
  - black
  - white
`},after:{schema:`$ref: "#/components/schemas/Color"
...
maximum: 99
`},after:{schema:`type: number
`}}],["remove-min-items-for-array-property",{before:{schema:`properties:
  option1:
    type: array
    minItems: 1
    items:
...
`},after:{schema:`properties:
  ...
  option2:
    type: string
`}}],["remove-min-properties-for-object-property",{before:{schema:`properties:
  option1:
    minProperties: 1
    properties:
      prop1:
...
type: string
`},after:{schema:`type: string
`}}],["remove-sibling-maxLength-for-ref",{before:{schema:`$ref: "#/components/schemas/Color"
maxLength: 10
`},after:{schema:`$ref: "#/components/schemas/Color"
...
xml:
  prefix: "p2"
  namespace: "http://example.com/schema"
`}}]]),k=new Map([["openapi/parameters-schema",`openapi: 3.0.4
info:
        ...
        "200":
          ...
          headers:
            X-Header-1:
              schema:
                __SCHEMA__
`]]),U="openapi",fn="graphql",W="/",T=(n,e,t)=>[n,e,t].join(W),L=n=>n.replace(/\r\n/g,`
`),ln=(n,e)=>{const t=L(n).trimEnd();return t?t.split(`
`).map(o=>o.length?`${e}${o}`:o).join(`
`):""},dn=(n,e,t)=>{var b;const o=L(n),s=o.split(`
`),a=s.findIndex(I=>I.trim()===e);if(a===-1)return o;const c=((b=s[a].match(/^\s*/))==null?void 0:b[0])??"",g=ln(t,c);return g?(s.splice(a,1,...g.split(`
`)),s.join(`
`)):(s.splice(a,1),s.join(`
`))},yn=(n,e)=>{const t=dn(n,"__SCHEMA__",e.schema);return`${L(t).trimEnd()}
`},hn=W,V=(n,e)=>[n,e].join(hn),z=(n,e)=>k.has(V(n,e)),N=(n,e,t,o)=>{const s=V(n,e),a=k.get(s);if(!a)return"";const c=_.get(t);if(!c)return"";const g=c[o];return yn(a,g)},gn=["3.0.0","3.0.0"],bn=["unversioned","unversioned"],vn=(n,e,t)=>z(n,e)&&_.has(t),xn=(n,e,t)=>{const o=T(n,e,t),s=X.get(o);if(s)return[s.before,s.after];if(!z(n,e))return null;const a=N(n,e,t,"before"),c=N(n,e,t,"after");return!a&&!c?null:[a,c]},In={[U]:{defaultPair:gn,patchSamples:(n,e,t)=>[M(n,t[0]),M(e,t[1])]},[fn]:{defaultPair:bn}},H=n=>{const e=In[n];if(!e)throw new Error(`Unknown suiteType for VersionPairPolicy lookup: ${n}`);return e},Y=(n,e)=>{var t;if(n===U)return(t=un.get(e))==null?void 0:t.versionPairs},M=(n,e)=>{const t=/^openapi:\s*.*$/m;if(!t.test(n))throw new Error('Invalid OpenAPI sample: missing root "openapi" field');return n.replace(t,`openapi: ${e}`)},On=(n,e,t)=>{const o=T(n,e,t);if(!X.has(o)&&!vn(n,e,t))throw new Error(`Unknown compatibility suite case: (${n}, ${e}, ${t})`);const s=H(n),a=Y(n,o);return a||[s.defaultPair]},qn=(n,e,t,o)=>{const s=xn(n,e,t);if(!s)return["",""];if(o===void 0)return s;const a=H(n);if(!a.patchSamples)throw new Error(`specificationVersionPair is currently supported only for OpenAPI cases: (${n}, ${e}, ${t})`);if(!On(n,e,t).some(P=>P[0]===o[0]&&P[1]===o[1]))throw new Error(`Unsupported specificationVersionPair [${o[0]}, ${o[1]}] for case (${n}, ${e}, ${t})`);const b=T(n,e,t);return Y(n,b)?a.patchSamples(s[0],s[1],o):s};var J={exports:{}};(function(n){(function(){function e(r,i){document.addEventListener?r.addEventListener("scroll",i,!1):r.attachEvent("scroll",i)}function t(r){document.body?r():document.addEventListener?document.addEventListener("DOMContentLoaded",function i(){document.removeEventListener("DOMContentLoaded",i),r()}):document.attachEvent("onreadystatechange",function i(){(document.readyState=="interactive"||document.readyState=="complete")&&(document.detachEvent("onreadystatechange",i),r())})}function o(r){this.g=document.createElement("div"),this.g.setAttribute("aria-hidden","true"),this.g.appendChild(document.createTextNode(r)),this.h=document.createElement("span"),this.i=document.createElement("span"),this.m=document.createElement("span"),this.j=document.createElement("span"),this.l=-1,this.h.style.cssText="max-width:none;display:inline-block;position:absolute;height:100%;width:100%;overflow:scroll;font-size:16px;",this.i.style.cssText="max-width:none;display:inline-block;position:absolute;height:100%;width:100%;overflow:scroll;font-size:16px;",this.j.style.cssText="max-width:none;display:inline-block;position:absolute;height:100%;width:100%;overflow:scroll;font-size:16px;",this.m.style.cssText="display:inline-block;width:200%;height:200%;font-size:16px;max-width:none;",this.h.appendChild(this.m),this.i.appendChild(this.j),this.g.appendChild(this.h),this.g.appendChild(this.i)}function s(r,i){r.g.style.cssText="max-width:none;min-width:20px;min-height:20px;display:inline-block;overflow:hidden;position:absolute;width:auto;margin:0;padding:0;top:-999px;white-space:nowrap;font-synthesis:none;font:"+i+";"}function a(r){var i=r.g.offsetWidth,p=i+100;return r.j.style.width=p+"px",r.i.scrollLeft=p,r.h.scrollLeft=r.h.scrollWidth+100,r.l!==i?(r.l=i,!0):!1}function c(r,i){function p(){var u=v;a(u)&&u.g.parentNode!==null&&i(u.l)}var v=r;e(r.h,p),e(r.i,p),a(r)}function g(r,i,p){i=i||{},p=p||window,this.family=r,this.style=i.style||"normal",this.weight=i.weight||"normal",this.stretch=i.stretch||"normal",this.context=p}var b=null,I=null,P=null,D=null;function Z(r){return I===null&&(G(r)&&/Apple/.test(window.navigator.vendor)?(r=/AppleWebKit\/([0-9]+)(?:\.([0-9]+))(?:\.([0-9]+))/.exec(window.navigator.userAgent),I=!!r&&603>parseInt(r[1],10)):I=!1),I}function G(r){return D===null&&(D=!!r.document.fonts),D}function O(r,i){var p=r.style,v=r.weight;if(P===null){var u=document.createElement("div");try{u.style.font="condensed 100px sans-serif"}catch{}P=u.style.font!==""}return[p,v,P?r.stretch:"","100px",i].join(" ")}g.prototype.load=function(r,i){var p=this,v=r||"BESbswy",u=0,q=i||3e3,E=new Date().getTime();return new Promise(function($,Q){if(G(p.context)&&!Z(p.context)){var nn=new Promise(function(S,j){function l(){new Date().getTime()-E>=q?j(Error(""+q+"ms timeout exceeded")):p.context.document.fonts.load(O(p,'"'+p.family+'"'),v).then(function(x){1<=x.length?S():setTimeout(l,25)},j)}l()}),en=new Promise(function(S,j){u=setTimeout(function(){j(Error(""+q+"ms timeout exceeded"))},q)});Promise.race([en,nn]).then(function(){clearTimeout(u),$(p)},Q)}else t(function(){function S(){var m;(m=d!=-1&&y!=-1||d!=-1&&h!=-1||y!=-1&&h!=-1)&&((m=d!=y&&d!=h&&y!=h)||(b===null&&(m=/AppleWebKit\/([0-9]+)(?:\.([0-9]+))/.exec(window.navigator.userAgent),b=!!m&&(536>parseInt(m[1],10)||parseInt(m[1],10)===536&&11>=parseInt(m[2],10))),m=b&&(d==F&&y==F&&h==F||d==w&&y==w&&h==w||d==A&&y==A&&h==A)),m=!m),m&&(f.parentNode!==null&&f.parentNode.removeChild(f),clearTimeout(u),$(p))}function j(){if(new Date().getTime()-E>=q)f.parentNode!==null&&f.parentNode.removeChild(f),Q(Error(""+q+"ms timeout exceeded"));else{var m=p.context.document.hidden;(m===!0||m===void 0)&&(d=l.g.offsetWidth,y=x.g.offsetWidth,h=K.g.offsetWidth,S()),u=setTimeout(j,50)}}var l=new o(v),x=new o(v),K=new o(v),d=-1,y=-1,h=-1,F=-1,w=-1,A=-1,f=document.createElement("div");f.dir="ltr",s(l,O(p,"sans-serif")),s(x,O(p,"serif")),s(K,O(p,"monospace")),f.appendChild(l.g),f.appendChild(x.g),f.appendChild(K.g),p.context.document.body.appendChild(f),F=l.g.offsetWidth,w=x.g.offsetWidth,A=K.g.offsetWidth,j(),c(l,function(m){d=m,S()}),s(l,O(p,'"'+p.family+'",sans-serif')),c(x,function(m){y=m,S()}),s(x,O(p,'"'+p.family+'",serif')),c(K,function(m){h=m,S()}),s(K,O(p,'"'+p.family+'",monospace'))})})},n.exports=g})()})(J);var Sn=J.exports;const jn=an(Sn),Pn=Symbol("synthetic-title"),Kn={validate:!0,liftCombiners:!0,syntheticTitleFlag:Pn,unify:!0,allowNotValidSyntheticChanges:!0},Fn=(n,e)=>{const s=tn(n,e,{...Kn,beforeSource:n,afterSource:e,mode:rn,metaKey:C});return wn(s.merged)&&(s.merged.toJSON=()=>sn(s.merged)),s};function wn(n){return n!=null&&typeof n=="object"}const An=["Inter"],Dn={diffsMetaKey:C,aggregatedDiffsMetaKey:on};function Bn({before:n,after:e}){const{merged:t}=Fn(R(n),R(e)),[o,s]=mn.useState(!1),a=An.map(c=>new jn(c).load(null,1e4));return Promise.all(a).then(()=>{s(!0)}),o?B.jsx(cn,{layoutMode:pn,metaKeys:Dn,source:t}):B.jsx(B.Fragment,{})}function Qn(n,e,t){const[o,s]=qn(n,e,t);return{before:o,after:s}}Bn.__docgenInfo={description:"",methods:[],displayName:"GraphQLStoryComponent",props:{before:{required:!0,tsType:{name:"string"},description:""},after:{required:!0,tsType:{name:"string"},description:""}}};export{Bn as G,fn as T,Qn as g};
