        ...
        '200':
          ...
          headers:
            X-Header-1:
              schema:
                type: [string, "null"]
`}]]),Qn=Symbol("synthetic-title"),Mn={validate:!0,liftCombiners:!0,syntheticTitleFlag:Qn,unify:!0,allowNotValidSyntheticChanges:!0},Gn=(n,t)=>{const e=un(n,t,{...Mn,beforeSource:n,afterSource:t,mode:dn,metaKey:sn});return _n(e.merged)&&(e.merged.toJSON=()=>pn(e.merged)),e};function _n(n){return n!=null&&typeof n=="object"}const Xn=["Inter"];function Un({before:n,after:t}){const{diffs:s,merged:o}=Gn(H(n),H(t)),[e,p]=bn.useState(!1),c=Xn.map(a=>new Ln(a).load(null,1e4));return Promise.all(c).then(()=>{p(!0)}),e?(console.log("apihub-diff:",pn(s.map(a=>{const{beforeDeclarationPaths:y=[],afterDeclarationPaths:h=[],...w}={...a};return{...w,...$.isNotEmpty(y)?{beforeDeclarationPaths:`[${y.map(I=>`[${I.join()}]`).join()}]`}:{},...$.isNotEmpty(h)?{afterDeclarationPaths:`[${h.map(I=>`[${I.join()}]`).join()}]`}:{}}}))),Cn(n,t),M.jsx(vn,{layoutMode:hn,diffMetaKey:sn,source:o})):M.jsx(M.Fragment,{})}function Cn(n,t){const s=J(n,{noLocation:!0}),o=J(t,{noLocation:!0}),e=Pn(s,o),p=Fn(s,o);console.log("GraphQL Changes:"),$.isNotEmpty(e)?console.log("Breaking Changes:",e.map(c=>`
${c.description}`).join()):console.log("No Breaking Changes"),$.isNotEmpty(p)?console.log("Dangerous Changes:",p.map(c=>`
${c.description}`).join()):console.log("No Dangerous Changes")}function Zn(n,t,s){const[o,e]=Nn(n,t,s);return{before:o,after:e}}Un.__docgenInfo={description:"",methods:[],displayName:"GraphQLStoryComponent",props:{before:{required:!0,tsType:{name:"string"},description:""},after:{required:!0,tsType:{name:"string"},description:""}}};export{Un as G,Jn as T,Zn as g};
